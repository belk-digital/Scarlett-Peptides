...
const FAQ_CATEGORIES: FaqCategory[] = [
    ...
    faqs: [
      {
        ...
        a: <>Payment is processed entirely within <a href="http://99puritypeptides.com/" target="_blank" rel="noopener noreferrer" className="underline hover:text-white transition-colors">99 Purity Peptides</a>' secure checkout infrastructure. Scarlett Hawkins does not handle, process, or store any payment data. Their checkout uses industry-standard SSL encryption.</>
      },
      {
  ...
  },
  {
    ...
    faqs: [
      {
        q: "What purity standard do Scarlett Hawkins products meet?",
        a: "Every compound in the Scarlett Hawkins catalog is independently verified to a minimum of ≥99% purity by HPLC (High-Performance Liquid Chromatography), with molecular identity confirmed by LC-MS (Liquid Chromatography–Mass Spectrometry). These standards apply to every batch, without exception."
      },
      {
        q: "What is a Certificate of Analysis (COA)?",
        a: <>A COA is a formal laboratory document issued by an independent testing facility that details the exact purity percentage of a compound, the testing method used, the impurity profile, and confirmation of molecular identity. Every Scarlett Hawkins product ships with a batch-specific COA, identifiable by the batch number printed on your vial. All COAs are published at <a href="https://99puritypeptides.com/certificates" target="_blank" rel="noopener noreferrer" className="underline hover:text-white transition-colors">99puritypeptides.com/certificates</a>.</>
      },
      {
      ...
      },
      {
        ...
        a: "Testing is performed by an independent, US-based third-party laboratory with no commercial affiliation to the manufacturer or to Scarlett Hawkins. This independence is a core requirement of our purity standard — we do not accept self-reported or manufacturer-conducted test results."
      },
      {
  ...
  },
  {
    ...
    faqs: [
      ...
      },
      {
        ...
        a: "Redistribution of Scarlett Hawkins products for non-research purposes is strictly prohibited. These compounds are sold under Research Use Only classification and may only change hands in compliance with applicable research compound regulations."
      },
      {
        q: "What happens if my country has restrictions on importing research peptides?",
        a: "Import regulations for research compounds vary significantly by country. Buyers are solely responsible for understanding and complying with the applicable laws in their jurisdiction. Scarlett Hawkins and 99 Purity Peptides are not responsible for any customs delays, seizures, or regulatory complications resulting from international orders."
      }
    ]
  },
  {
            ...
            <nav className="flex flex-col space-y-1">
              {FAQ_CATEGORIES.map(cat => (
                <a
                  key={cat.id}
                  href={`#${cat.id}`}
                  className="text-textsub hover:text-white text-sm py-2 px-3 rounded-lg hover:bg-white/5 transition-all duration-200"
                >
                  {cat.title}
                </a>
              ))}
